...
@section('content')
    <div class="w-full">
        <div class="bg-white rounded-xl border border-gray-200 p-6 relative">
            <h2 class="text-base font-semibold text-gray-800 mb-5">Edit Video</h2>
            ...
            <div id="upload-overlay"
                ...
                <p class="text-sm font-medium text-gray-700">Uploading video...</p>
                <p class="text-xs text-gray-400">This may take a while for large files</p>
            </div>

            <form id="video-form" method="POST" action="{{ route('videos.update', $video) }}" enctype="multipart/form-data"
                class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full px-4 py-2.5 rounded-xl border border-gray-300 text-sm bg-gray-50
                               focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">{{ old('description', $video->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div id="thumbnail-preview-container" class="{{ $video->thumbnail ? '' : 'hidden' }} mb-2">
                        <img id="thumbnail-preview"
                            src="{{ $video->thumbnail ? asset('storage/' . $video->thumbnail) : '' }}"
                            alt="Thumbnail preview" class="w-40 h-24 object-cover rounded-lg border border-gray-200">
                    </div>
                ...
                <div>
                    <label for="video" class="block text-sm font-medium text-gray-700 mb-1">Video File
                        <span class="text-gray-400 font-normal">(leave empty to keep current)</span>
                    </label>
                    @if ($video->url)
                        <p class="mb-2 text-xs text-gray-500">
                            <i class="bi bi-file-earmark-play"></i> Current: <span
                                class="font-medium">{{ basename($video->url) }}</span>
                        </p>
                    @endif
    ...
    <script>
        ...
        document.getElementById('video-form').addEventListener('submit', function(e) {
            ...
            // Only show loading if a file is being uploaded
